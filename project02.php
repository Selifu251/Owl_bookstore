...
$query_Re_book_class = "SELECT * FROM owl_book_class ORDER BY obc_id ASC";
$Re_book_class = mysql_query($query_Re_book_class, $owl_bookstore) or die(mysql_error());
$totalRows_Re_book_class = mysql_num_rows($Re_book_class);
/* 查詢book_type */
mysql_select_db($database_owl_bookstore, $owl_bookstore);
$query_Re_book_type = "SELECT * FROM owl_book_type ORDER BY obt_id ASC";
$Re_book_type = mysql_query($query_Re_book_type, $owl_bookstore) or die(mysql_error());
$row_Re_book_type = mysql_fetch_assoc($Re_book_type);
$totalRows_Re_book_type = mysql_num_rows($Re_book_type);

if(isset($_POST['book_class'])){
	echo $_POST['book_class'];
}else{
	echo "失敗拉";
}
?>
			...
			<div class="agileits-navi_search">
				<form action="#" method="post">
					<select id="agileinfo-nav_search" name="agileinfo_search" required="">
						<option value="0">書籍分類</option>
                        <?php while($row_Re_book_class = mysql_fetch_assoc($Re_book_class)){ ?>
						<option value="<?php echo $row_Re_book_class['obc_id']; ?>"><?php echo $row_Re_book_class['obc_name']; ?></option>
                        <?php } ?>
					</select>
				</form>
			</div>
...
<div class="row">
      ...
      <div class="col-md-9">
       ...
       <div class="row">
        <!-- 1-1 -->
        <div class="col-md-4">
          <div class="thumbnail"> <a href="project_book.html"><img src="images/m2.jpg" /></a>
            <div class="caption">
              <h3></h3>
              <p>作者：</p>
              <p>價格:</p>
     ...
     <script>
      $(document).ready(function(e) {
        $('#agileinfo-nav_search').change(function(){
			$.post('project02.php',{name:"Jim"},function(data){ alert(data); });
		});
    });
     </script>
     <?php echo "POST=".$_POST['name']; ?>
    <!-- // Book_Class送出表單 -->
...
<?php
mysql_free_result($Re_book_type);

mysql_free_result($Re_book_class);
?>