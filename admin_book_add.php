...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "owl_member_profile")) {
	/* 圖檔上傳 */
	$insert_ok=0;
	$insert_img_ok=0;
	if(isset($_FILES['ob_img'])){
		...
		if($file_img['error']>0){
			if($file_img['error']==4){
				$book_img = 'bookimg/no_book_img.jpg';
				$insert_ok=1;
			}else{
	...
	/* // 圖檔上傳 */
	if($_POST['ob_name']=="" || $_POST['ob_author']=="" || $_POST['ob_publishing']="" || $_POST['ISBN']="" || $_POST['ob_price']=""){
		echo "<script>alert('資料不齊全');</script>";
		$insert_ok=0;
	}
	if($insert_ok==1){
		$insertSQL = sprintf("INSERT INTO owl_book (ob_name, ob_author, ob_publishing, ob_publication_day, ISBN, ob_price, ob_img, ob_content) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)",
    ...
    <div class="container">
     ...
     <div class="col-md-3">
      <div class="panel panel-default">
       ...
       <!-- // PanelList -->
       <div class="panel-footer"><a class="btn btn-danger" href="<?php echo $logoutAction ?>">登出</a></div>
      </div>
     </div>
     <!-- // container-left -->
     <!-- container-right -->
     <div class="col-md-8">
      ...
      <form action="<?php echo $editFormAction; ?>" method="POST" id="owl_member_profile" name="owl_member_profile" enctype="multipart/form-data">
       <tr style="background-color:#6CF">
        <td class="text-center" colspan="3"><h3>新增書籍</h3></td>
       </tr>
       <tr>
        <td class="col-md-2 text-right">書名：</td>
        <td class="col-md-10"  colspan="2"><input name="ob_name" type="text" class="form-control" id="ob_name"></td>
       </tr>
       <tr>
        <td class="col-md-2 text-right">作者：</td>
        <td class="col-md-10"  colspan="2"><input name="ob_author" type="text" class="form-control" id="ob_author"></td>
       </tr>
       <tr>
        <td class="col-md-2 text-right">簡介：</td>
        <td class="col-md-10"  colspan="2"><textarea style="height:200px;" name="ob_content" type="text" class="form-control" id="ob_content"></textarea></td>
       </tr>
       <tr>
        <td class="col-md-2 text-right">封面圖片：</td>
        <td class="col-md-3"><input type="file" id="ob_img" name="ob_img"></td>
        <td rowspan="4"><img src="bookimg/no_book_img.jpg" id="ob_img_change" class="img-responsive"></td>
       </tr>
       <tr>
        <td class="col-md-2 text-right">出版社：</td>
        <td class="col-md-3"><input name="ob_publishing" type="text" id="ob_publishing"></td>
       </tr>
       <tr>
        <td class="col-md-2 text-right">價格：</td>
        <td class="col-md-3"><input name="ob_price" type="text" id="ob_price"></td>
       </tr>
       <tr>
        <td class="col-md-2 text-right">ISBN：</td>
        <td class="col-md-3"><input name="ISBN" type="text" id="ISBN"></td>
       </tr>
       <tr>
        <td class="text-center" colspan="3">
         <input class="btn btn-success" type="submit" name="Submit" value="新增書籍">
         <input name="ob_publication_day" type="hidden" id="ob_publication_day" value="<?php echo date("Y-m-d"); ?>">
        </td>
       </tr>
       <input type="hidden" name="MM_insert" value="owl_member_profile">
      </form>
      </table>
      <!-- // 新增書籍 -->
     </div>
